...
class Bestiary():
    ...
    def build_bestiary(self, bestiary_url):
        try:
            self.bestiary = {beast['name'].lower(): beast for beast in \
    ...
    def create_embed(beast):
        ...
        inline_field('Size'); inline_field('Alignment'); inline_field('Speed')
        inline_field('AC'); inline_field('HP'); inline_field('CR')
        section_title('Stats')
        inline_field('STR'); inline_field('DEX'); inline_field('CON')
        inline_field('INT'); inline_field('WIS'); inline_field('CHA')
        ...
        if beast['saves'] is not None:
            e.add_field(
                name='Saving Throws',
                value=', '.join(
                    [f'{s.capitalize()} {beast["saves"][s]}' for s in beast['saves']]
                ),
                inline=False
            )
        if beast['skills'] is not None:
            e.add_field(
                name='Skills',
                value=', '.join(
                    [f'{s.capitalize()} {beast["skills"][s]}' for s in beast['skills']]
                ),
                inline=False
            )
        ...
        field_sections = [(s, s.lower().replace(' ', '_')) for s in \
            ['Traits', 'Actions', 'Legendary Actions']]
        if \
            sum(
                [len(beast[k]) if beast[k] != None else 0 for _, k in field_sections]
            ) + len(e.fields) < 25:
