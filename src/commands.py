...
class About(Command):
    ...
    async def handle(self, _):
        return (
            "Hi, I'm Owen's bot! I can help you in a variety of ways."
...
class Blackletter(Command):
    ...
    async def handle(self, message):
        ...
        if not argument:
            return (
                "Usage: `--bl <message>` to create "
                + wordart.blackletter("blackletter")
                + " text."
            )
...
class Help(Command):
    ...
    async def _handle(self, argument):
        ...
        suggestion = difflib.get_close_matches(
            argument.lower(), self.help_strings.keys(), 1
        )
        if suggestion:
            return (
                f"I couldn't find help for `{argument}`."
...
class VaporWave(Command):
    ...
    async def handle(self, message):
        ...
        if not argument:
            return (
                "Usage: `--vw <message>` to create "
                + wordart.vaporwave("vaporwave")
                + " text."
            )
...
class WordArt(Command):
    def __init__(self, config):
        super().__init__(config, commands=["--wa", "--wordart"], will_send=True)
        ...
        if argument:
            try:
                await message.channel.send(
                    wordart.handle_wordart_request(argument, self.default_emoji)
                )
                self.delete_message = True
...
class XKCD(Command):
    ...
    async def _handle(self, argument):
        ...
        return (
            "Use `--xkcd <comic name>` or `--xkcd <number>` to find an"
