...
class xkcd():
    ...
    def get_uri_alt(self):
        ...
        uri = re.search(r'https://imgs\.xkcd\.com/comics/[\w.()-]+\n', src) # Pull the permalink
        if uri:
            uri = uri.group(0)
            uri = uri[:len(uri) - 1] # Cut off the newline character
            self.uri = uri

        alt = re.search(r'<img src=\"[^\"]+\" title=\"[^\"]+\"', src) # Pull the alt text        
        if alt:
            alt = re.search('title="[^"]+"', alt.group(0)).group(0) # Grab the title section of the line
...
async def update_db(loop):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        calls = [loop.run_in_executor(executor, strip.get_uri_alt) \
            for strip in xkcds]
...
def start_db_thread(interval, client):
    thread = \
        threading.Thread(target=update_db_schedule, args=[interval, client])
    thread.start()
