...
class CardList():
    def __init__(self, cards, message, total_cards=None):
        ...
        self.total_cards = total_cards if total_cards is not None else \
            len(self.cards)
        self._results = None
...
class ScryfallHandler(commands.Pattern):
    ...
    async def handle_reaction(self, reaction, _):
        ...
        embed_style = 'thumbnail' if emoji in ScryfallHandler.SHRINK_EMOJIS \
            else 'full' if emoji in ScryfallHandler.ENLARGE_EMOJIS else None
        index = int(emoji[-1:]) - 1 if \
            emoji.startswith('keycap_') and 0 < int(emoji[-1]) < 6 else None
    ...
    def log_sent(self, message, content):
        ...
        if len(self.sent_channels[message.channel.id]) > \
    ...
    async def send(self, content, channel):
        if type(content) == str:
            ...
            self.log_sent(
                await channel.send(embed=content.get_embed()),
                content
            )
